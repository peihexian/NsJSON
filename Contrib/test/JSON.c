...
static PTCHAR CaptureComments(PTCHAR pszBuffer, int* piPos) {
	int iStart = *piPos-1;
	BOOL isComment = FALSE;

	if (pszBuffer[iStart] == TEXT('/') && pszBuffer[iStart + 1] == TEXT('/')) {
		*piPos = iStart;
		while (pszBuffer[*piPos] != TEXT('\n') && pszBuffer[*piPos] != 0) {
			(*piPos)++;
		}
		isComment = TRUE;
	}
	else if (pszBuffer[iStart] == TEXT('/') && pszBuffer[iStart + 1] == TEXT('*')) {
		*piPos = iStart;
		while (pszBuffer[*piPos] != 0 && !(pszBuffer[*piPos] == TEXT('*') && pszBuffer[*piPos + 1] == TEXT('/'))) {
			(*piPos)++;
		}
		if (pszBuffer[*piPos] == TEXT('*')) {
			*piPos += 2;
	...
	if (isComment) {
		return MakeCopy(pszBuffer, iStart, *piPos - 1);
	}
	return NULL;
}
...
static struct JSON_NODE* EatNode(PTCHAR pszBuffer, int* piPos, BOOL bIsValue)
{
	...
	if (pNode)
	{
		...
		if (EatChar(pszBuffer, piPos, TEXT('#')) || EatChar(pszBuffer, piPos, TEXT('/')))
		{
			...
			pNode->eType = JNT_COMMENT;
			PTCHAR commentData= CaptureComments(pszBuffer, piPos);
			pNode->pszKey = commentData;
			pNode->pszValue = commentData;
...
void JSON_Delete(struct JSON_NODE** ppNode, struct JSON_NODE* pPrev)
{
	if (ppNode && *ppNode)
	{
		...
		if (pPrev)
		{
			...
			if ((pPrev->eType == JNT_NODE || pPrev->eType == JNT_COMMENT || pPrev->eType == JNT_ARRAY) && pPrev->pValue == *ppNode)
...
struct JSON_NODE* Parse(PBYTE pbSource, enum JSON_SET_FLAGS eFlags)
{
...
struct JSON_NODE* JSON_Next(struct JSON_NODE** ppNode, PTCHAR pszKey, BOOL bKeyIsIndex, BOOL bCreate, BOOL* pbCreated)
{
	...
	if (ppNode)
	{
		...
		if (pszKey)
		{
			...
			if (pParent->eType == JNT_NODE || pParent->eType == JNT_ARRAY || pParent->eType == JNT_COMMENT)
			{
